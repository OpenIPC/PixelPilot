...
public:
    ...
    AndroidLogger(const android_LogPriority priority, const std::string TAG) : M_PRIORITY(priority),
                                                                               M_TAG(std::move(
                                                                                       TAG)) {}
...
private:
    ...
    void logBigMessage(const std::string &message) {
        if (message.length() > ANDROID_LOG_BUFF_SIZE) {
            __android_log_print(M_PRIORITY, M_TAG.c_str(), "%s",
                                message.substr(0, ANDROID_LOG_BUFF_SIZE).c_str());
...
namespace PrettyFunctionHelper {
     ...
     */
    static std::string
    namespaceAndClassName(const std::string &function, const std::string &prettyFunction) {
        ...
        const std::string namespaceAndClassName = returnTypeAndNamespaceAndClassName.substr(
                begin1 + 1);
    ...
    public:
        static std::string testMacro(std::string unused) {
            const auto namespaceAndClassName = PrettyFunctionHelper::namespaceAndClassName(
            ...
            public:
                static std::string testMacro() {
                    const auto namespaceAndClassName = PrettyFunctionHelper::namespaceAndClassName(
                            __FUNCTION__, __PRETTY_FUNCTION__);
                    AndroidLogger(ANDROID_LOG_DEBUG, "NoT2") << namespaceAndClassName;
                    assert(namespaceAndClassName.compare(
                            "PrettyFunctionHelper::TestNamespace1::TestNamespace2::Test2") == 0);
                    const auto className = PrettyFunctionHelper::className(namespaceAndClassName);
