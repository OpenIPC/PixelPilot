...
typedef std::function<void(const std::chrono::steady_clock::time_point creation_time,
                           const uint8_t *nalu_data,
                           const int nalu_data_size)> RTP_FRAME_DATA_CALLBACK;
...
private:
    ...
    void h265_forward_one_nalu(const uint8_t *data, int data_size,
                               bool write_4_bytes_for_start_code = true);

    // wtf
    static bool check_has_valid_prefix(const uint8_t *nalu_data, int nalu_data_len,
