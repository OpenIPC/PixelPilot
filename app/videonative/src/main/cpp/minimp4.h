...
*/
MP4D_file_offset_t
MP4D_frame_offset(const MP4D_demux_t *mp4, unsigned int ntrack, unsigned int nsample,
                  unsigned int *frame_bytes, unsigned *timestamp, unsigned *duration);
...
MP4E_mux_t *MP4E_open(int sequential_mode_flag, int enable_fragmentation, void *token,
                      int (*write_callback)(int64_t offset, const void *buffer, size_t size,
                                            void *token));
...
*/
int MP4E_put_sample(MP4E_mux_t *mux, int track_num, const void *data, int data_bytes, int duration,
                    int kind);
...
static int write_pending_data(MP4E_mux_t *mux, track_t *tr) {
    ...
    if (tr->pending_sample.bytes > 0 && tr->smpl.bytes >= sizeof(sample_t)) {
        ...
        // Write data
        ERR(mux->write_callback(mux->write_pos, tr->pending_sample.data, tr->pending_sample.bytes,
                                mux->token));
...
*/
static int
mp4e_write_fragment_header(MP4E_mux_t *mux, int track_num, int data_bytes, int duration, int kind
...
#endif
        if (!mux->fragments_count++) ERR(
                mp4e_flush_index(mux)); // write file headers before 1st sample
...
#define MOOV_TIMESCALE 1000
    ...
    for (ntr = 0; ntr < ntracks; ntr++) {
        ...
        {
            int lang_code =
                    ((tr->info.language[0] & 31) << 10) | ((tr->info.language[1] & 31) << 5) |
                    (tr->info.language[2] & 31);
...
static int transcode_nalu(h264_sps_id_patcher_t *h, const unsigned char *src, int nalu_bytes,
                          unsigned char *dst) {
    int old_id;
...
static int mp4_h265_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int sizeof_nal,
                              unsigned timeStamp90kHz_next) {
    ...
    switch (payload_type) {
        ...
        default:
            ...
            }
            {
                ...
                err = MP4E_put_sample(h->mux, h->mux_track_id, tmp, 4 + sizeof_nal,
                                      timeStamp90kHz_next, sample_kind);
                free(tmp);
            }
            break;
    }
            ...
            default:
                ...
                if (!h->need_pps && !h->need_idr) {
                    ...
                    if (first_mb_in_slice)
                        ...
                        sample_kind = MP4E_SAMPLE_RANDOM_ACCESS;
                    err = MP4E_put_sample(h->mux, h->mux_track_id, nal2, sizeof_nal,
                                          timeStamp90kHz_next, sample_kind);
                }
                break;
        }
    ...
    do {
        ...
        static const struct {
            ...
            unsigned use_track_flag;
        } g_fullbox[] =
                {
                        ...
                        {BOX_stsd, 0, 0},
                        {BOX_esds, 0,
                         1}    // esds does not use track, but switches to OD mode. Check here, to avoid OD check
                };
        ...
        if (depth) {
            ...
            if (box_bytes > stack[depth].bytes) {
                TRACE(("Wrong %c%c%c%c box size: broken file?\n", (box_name >> 24) & 255,
                        (box_name >> 16) & 255, (box_name >> 8) & 255, box_name & 255));
        ...
        switch (box_name) {
            ...
            case BOX_stsc:  //ISO/IEC 14496-12 Page 38. Section 8.18 - Sample To Chunk Box.
                ...
                MALLOC(MP4D_sample_to_chunk_t*, tr->sample_to_chunk,
            ...
            case BOX_stts: {
                ...
                for (i = 0; i < count; i++) {
                    ...
                    if (k + sc > ts_count) {
                        ts_count = k + sc;
                        tr->timestamp = (unsigned int *) realloc(tr->timestamp,
                                                                 ts_count * sizeof(unsigned));
                        tr->duration = (unsigned int *) realloc(tr->duration,
                                                                ts_count * sizeof(unsigned));
                    }
                    for (j = 0; j < sc; j++) {
                        tr->duration[k] = d;
            ...
            case BOX_co64:
                ...
                MALLOC(MP4D_file_offset_t*, tr->chunk_offset,
...
#if MP4D_INFO_SUPPORTED
            case BOX_mvhd: SKIP(((FullAtomVersionAndFlags >> 24) == 1) ? 8 + 8 : 4 + 4);
            ...
            case BOX_mdhd: SKIP(((FullAtomVersionAndFlags >> 24) == 1) ? 8 + 8 : 4 + 4);
                tr->timescale = READ(4);
            ...
            case BOX_stsd: SKIP(4); // entry_count, BOX_mp4a & BOX_mp4v boxes follows immediately
                break;
...
//         case BOX_svc1:   - no test
            ...
            default:
                TRACE(("[%c%c%c%c]  %d\n", box_name >> 24, box_name >> 16, box_name
                        >> 8, box_name, (int) payload_bytes));
        }
...
MP4D_file_offset_t
MP4D_frame_offset(const MP4D_demux_t *mp4, unsigned ntrack, unsigned nsample, unsigned *frame_bytes,
...
static const void *
MP4D_read_spspps(const MP4D_demux_t *mp4, unsigned int ntrack, int pps_flag, int nsps,
                 int *sps_bytes) {
