...
RTPDecoder::RTPDecoder(RTP_FRAME_DATA_CALLBACK cb, bool feed_incomplete_frames) : m_cb(
        std::move(cb)), m_feed_incomplete_frames(feed_incomplete_frames) {
}
...
void RTPDecoder::h265_forward_one_nalu(const uint8_t *data, int data_size,
                                       bool write_4_bytes_for_start_code) {
    timePointStartOfReceivingNALU = std::chrono::steady_clock::now();
    if (flagPacketHasGoneMissing) {
        //MLOGD<<"Got full NALU - clearing missing packet flag";
        flagPacketHasGoneMissing = false;
...
void RTPDecoder::parseRTPH265toNALU(const uint8_t *rtp_data, const size_t data_length) {
    ...
    if (nal_unit_header_h265.type == 48) {
        ...
        if (fu_header.e) {
            ...
            uint8_t variableNoIdea = rtp_data[sizeof(rtp_header_t) +
                                              sizeof(nal_unit_header_h265_t)];
...
void RTPDecoder::append_nalu_data(const uint8_t *data, size_t data_len) {
    if (m_nalu_data_length + data_len > m_curr_nalu.size()) {
        MLOGD << "Weird - not enough space to write NALU. curr_size:" << m_nalu_data_length
              << " append:" << data_len;
        return;
    }
...
void RTPDecoder::append_empty(size_t data_len) {
    if (m_nalu_data_length + data_len > m_curr_nalu.size()) {
        MLOGD << "Weird - not enugh space to write NALU. curr_size:" << m_nalu_data_length
              << " append:" << data_len;
        return;
    }
    uint8_t *p = &m_curr_nalu.at(m_nalu_data_length);
...
bool RTPDecoder::check_has_valid_prefix(const uint8_t *nalu_data, int nalu_data_len,
